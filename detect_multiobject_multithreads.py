...
# run muti
    ...
    if os.path.isdir(source):
        for i in range(0,1):
            ...
            videowrite.append(cv2.VideoWriter(r'end'+str(i+1)+'.mp4',-1,20, (960,  540)))
            q.append(Queue())
            threads.append(Thread(target = multithreading, args = (cap,device,half,names,colors,model, q[i])))
            threads[i].start()
            threads[i].join()
        for k in range(0,q[0].qsize()):
            for j in range(0,1):
                img[j]=q[j].get()
                re_img0 = (int(img[j].shape[1]*rescale) ,int(img[j].shape[0]*rescale))
